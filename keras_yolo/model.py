...
class Yolo(Model):
    ...
    def base_anchor_boxes(self):
        ...
        # The resulting grid + 0.5 are the center points of the actual grid cells
        xy_grid = tf.tile(tf.concat([cell_x,cell_y], -1), [1, 1, len(self.anchors), 1]) + 0.5
    ...
    def _get_graph(self):
        ...
        # First we predict 4 coordinates per box to which we will apply the right transformations in
        # the Yolo.anchor_boxes method
        boxes = Conv2D(len(self.anchors) * 4, (1,1), strides=(1,1), padding='same', name='boxes', activation='linear')(x)
        boxes = Reshape((grid_h, grid_w, len(self.anchors), 4))(boxes)
        boxes = Lambda(self._anchor_boxes)(boxes)

        confidence = Conv2D(len(self.anchors) * 1, (1,1), strides=(1,1), padding='same', name='conf', activation='sigmoid')(x)
        confidence = Reshape((grid_h, grid_w, len(self.anchors), 1))(confidence)

        classes = Conv2D(self.n_labels, (1,1), strides=(1,1), padding='same', name='probas')(x)
    ...
    def _warmup_loss(self, y_true, y_pred):
        ...
        xy_loss = tf.reduce_sum(
            tf.squared_difference(xy_true, xy_pred))

        print_op = tf.print('Warm up loss:', xy_loss + wh_loss, output_stream=sys.stdout)

        with tf.control_dependencies([print_op]):
            loss = wh_loss + xy_loss
    ...
    def pad_base_boxes_for_loss(self):
        base_boxes = tf.tile(tf.expand_dims(self.base_anchor_boxes(), axis=-2),
    ...
    def build_iou_mask(self, iou, threshold=None):
        ...
        the best matching anchor of all anchors is returned. If a threshold is given
        all values with an iou above that threshold are returned
        Args:
            iou: Tensor of shape (None, GRID_H, GRID_W, N_ANCHORS, MAX_TRAIN_BOXES)
            threshold: (None or Float) If given, the IOU threshold to use for the mask
        Returns:
        ...
        if threshold is None:
            max_iou = tf.tile(tf.reduce_max(iou, axis=[1,2,3], keepdims=True),
    ...
    def _loss(self, y_true, y_pred):
        ...
        obj_conf_loss = tf.reduce_mean(tf.squared_difference(true_confs_masked, conf_pred_masked))

        # compute the no object confidence loss
        no_conf_pred_masked = tf.boolean_mask(conf_pred, ~(anchors_mask | predictions_mask))
        no_obj_conf_loss = tf.reduce_mean(tf.squared_difference(0., no_conf_pred_masked))

        print_op_xy = tf.print('XY Loss:', xy_loss, output_stream=sys.stdout)
        print_op_wh = tf.print('WH Loss:', wh_loss, output_stream=sys.stdout)
        print_op_conf = tf.print('Conf Loss:', obj_conf_loss, output_stream=sys.stdout)
        print_op_noconf = tf.print('No Conf Loss:', no_obj_conf_loss, output_stream=sys.stdout)

        with tf.control_dependencies([print_op_xy, print_op_wh, print_op_conf, print_op_noconf]):
            loss = coord_loss + obj_conf_loss + 0.5 * no_obj_conf_loss
    ...
    def loss(self, y_true, y_pred):
        ...
        loss = tf.cond(tf.less(seen, self.warmup_rounds),
